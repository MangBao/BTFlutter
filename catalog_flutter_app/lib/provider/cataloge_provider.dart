...
//trang thai ung dung
  ...
  void addToCatalog(MatHang mh) {
    mh.id = _matHang.length;
    mh.color = Colors.primaries[mh.id % Colors.primaries.length];
    mh.price = 25;
    _matHang.add(mh);
    notifyListeners();
  }
  void removeProduct(MatHang mh){
    int idDel = _matHang.indexOf(mh);
    cataloge.removeAt(idDel);
    notifyListeners();
  }
