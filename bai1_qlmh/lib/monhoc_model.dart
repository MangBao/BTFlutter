...
class MonHocSnapshot {
  ...
  MonHocSnapshot.fromSnapshot(DocumentSnapshot snapshot) {
    this.monHoc =
        MonHoc.fromJson(snapshot.data()); //data để trích xuất dl, trả về đt map
...
Stream<List<MonHocSnapshot>> getAllFromFirebase() {
  Stream<QuerySnapshot> querySnap =
      FirebaseFirestore.instance.collection("MonHoc").snapshots();
  Stream<List<DocumentSnapshot>> list =
      querySnap.map((qsn) => qsn.docs); //thực hiện trên mỗi Document
  Stream<List<MonHocSnapshot>> listUserSn = list.map((listDocSn) => listDocSn
      .map((docSnap) => MonHocSnapshot.fromSnapshot(docSnap))
      .toList());
  return listUserSn;
}
